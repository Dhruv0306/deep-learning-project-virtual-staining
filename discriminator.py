...
class PatchDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc=3):
        ...
        # Initial convolution layer without batch normalization to avoid artifacts
        model += [
        ...
        for n in range(1, 3):
            model += [
                ...
                nn.InstanceNorm2d(out_features),  # Instance normalization for better training stability
        ...
        # Additional layer to increase receptive field without further downsampling
        model += [
            nn.Conv2d(
                in_features,
                in_features * 2,
...
def getDiscriminators():
    ...
    This function creates two discriminators (D_A and D_B) for the two domains (unstained, stained)
    in CycleGAN, initializes their weights, and tests them with random input.
    
    Returns:
        tuple: A tuple containing (D_A, D_B) - the two discriminator networks
    """
