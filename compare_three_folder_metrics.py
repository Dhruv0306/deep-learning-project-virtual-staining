...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Batch size used for feature extraction when computing FID",
    )
    parser.add_argument(
        "--output_prefix",
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Device for metric computation. 'auto' uses CUDA when available.",
    )
    parser.add_argument(
        "--match_mode",
        ...
        default="exact",
        help=(
            "Filename matching mode. exact uses full filename (case-insensitive). "
            "prefix uses text before the first dot and is useful when generated files add suffixes."
        ),
    )
...
def to_normalized_tensor(image: Image.Image) -> torch.Tensor:
    tensor = torch.from_numpy(np.array(image).copy()).permute(2, 0, 1).float() / 255.0
...
def compute_ssim_torch(
    ...
    for ch in range(channels):
        ...
        mu_y = torch.nn.functional.avg_pool2d(
            y, kernel_size=kernel_size, stride=1, padding=pad
        )
        ...
        sigma_y_sq = (
            torch.nn.functional.avg_pool2d(
                y * y, kernel_size=kernel_size, stride=1, padding=pad
            )
            - mu_y_sq
        )
        sigma_xy = (
            torch.nn.functional.avg_pool2d(
                x * y, kernel_size=kernel_size, stride=1, padding=pad
            )
            - mu_xy
        )
...
def batched_inception_features(
    ...
    for i in range(0, len(tensors), batch_size):
        batch = torch.stack(list(tensors[i : i + batch_size]), dim=0)
...
def main() -> None:
    ...
    for match_key in matched_keys:
        ...
        per_image_rows.append(
            PairMetrics(
                ...
                ssim_src_gen1=float(compute_ssim_torch(src_b, gen1_b)),
                psnr_src_gen1=float(compute_psnr_torch(src_b, gen1_b)),
                mae_src_gen1=float(compute_mae(src_b, gen1_b)),
                ssim_src_gen2=float(compute_ssim_torch(src_b, gen2_b)),
                psnr_src_gen2=float(compute_psnr_torch(src_b, gen2_b)),
                mae_src_gen2=float(compute_mae(src_b, gen2_b)),
            )
        )
    ...
    gen1_features = batched_inception_features(
        calculator, gen1_tensors, args.batch_size
    )
    gen2_features = batched_inception_features(
        calculator, gen2_tensors, args.batch_size
    )
